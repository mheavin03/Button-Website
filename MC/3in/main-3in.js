...
(function(){
  ...
  var SHOW_INNER_RING_IN_PDF = false;
  ...
  // Set 3"/3.451" as defaults
  setVar('--cols', COLS);
  setVar('--rows', ROWS);
   // Set 3"/3.451" as defaults
   setVar('--inner-dia-in', 3);
   setVar('--outer-dia-in', 3.451);
   setVar('--inner-dia', 'calc(3 * 96px)');
   setVar('--outer-dia', 'calc(3.451 * 96px)');
  // Default gaps (independent X/Y)
  setVar('--gap-x-in', 0.65);
  setVar('--gap-y-in', 0.15);
  setVar('--gap-x', 'calc(0.65 * 96px)');
  setVar('--gap-y', 'calc(0.15 * 96px)');
  // Legacy single gap for fallback
  setVar('--gap-in', 0.15);
  ...
  function buildGridForPage(page){
    ...
    for(var i=0; i<page.state.length; i++){
      (function(i){
        ...
        cell.appendChild(tb);
        tb.addEventListener('mousedown', function(e){ if (e.target && e.target.tagName === 'BUTTON') e.preventDefault(); e.stopPropagation(); });
        ...
        tb.insertBefore(zoomOutBtn, tb.lastChild);
        tb.insertBefore(zoomInBtn, tb.lastChild);
        tb.insertBefore(centerBtn, tb.lastChild);
    ...
    cell.appendChild(dupBtn);
        ...
        (function(){
          ...
          imgEl.addEventListener('pointermove', function(ev){
            ...
            var innerDiaStr = getComputedStyle(root).getPropertyValue('--inner-dia');
            var previewScaleStr = getComputedStyle(root).getPropertyValue('--preview-scale');
            var innerPx = parsePxVar(innerDiaStr, 192);
            var previewScale = parseFloat(previewScaleStr) || 0.9;
            var denom = innerPx * previewScale;
  ...
  function applySettings(){
    ...
    // legacy for other styles that still read --gap
  ...
  function showFilenamePrompt(initialName){
    return new Promise(function(resolve){
      var existing = document.getElementById('filenamePromptOverlay'); if (existing) existing.remove();
      var overlay = document.createElement('div'); overlay.id = 'filenamePromptOverlay'; overlay.className = 'modal-overlay';
      ...
      var input = modal.querySelector('#filenamePromptInput'); var ok = modal.querySelector('.btn.ok'); var cancel = modal.querySelector('.btn.cancel');
      input.value = initialName || 'export.pdf'; setTimeout(function(){ try{ input.focus(); input.select(); }catch(_){ } }, 0);
      ...
      function abort(){ resolve(null); cleanup(); }
      ok.addEventListener('click', confirm); cancel.addEventListener('click', abort);
      overlay.addEventListener('click', function(e){ if(e.target === overlay) abort(); });
      input.addEventListener('keydown', function(e){ if(e.key === 'Enter'){ e.preventDefault(); confirm(); } else if(e.key === 'Escape'){ e.preventDefault(); abort(); } });
    });
  }

  document.getElementById('exportPDF').addEventListener('click', function(){
    var proposed = getExportFilename('circle-template-3x4-3in.pdf');
  ...
  var y = g.marginY + r*(g.outerDiaPx + g.gapYPx);
        ...
        if(cellState.img){
          loadImage(cellState.img).then(function(img){
            ...
            var box = coverContainRect(img.width, img.height, g.innerDiaPx, g.innerDiaPx, fit);
            var s = cellState.scale || 1; var nx = cellState.nx || 0; var ny = cellState.ny || 0;
            var dw = box.dWidth * s; var dh = box.dHeight * s;
            var dx = cx - dw/2 + nx * g.innerDiaPx; var dy = cy - dh/2 + ny * g.innerDiaPx;
            ctx.drawImage(img, box.sx, box.sy, box.sWidth, box.sHeight, dx, dy, dw, dh);
            ctx.restore();
            ctx.lineWidth = 2;
            ctx.strokeStyle = '#2a49d0'; ctx.beginPath(); ctx.arc(cx, cy, outerR, 0, Math.PI * 2); ctx.stroke();
            if (SHOW_INNER_RING_IN_PDF) { ctx.strokeStyle = '#e63c3c'; ctx.beginPath(); ctx.arc(cx, cy, innerR, 0, Math.PI * 2); ctx.stroke(); }
  ...
  // Fallback if not set
  if (!gapXIn && gapXIn !== 0) gapXIn = parseFloat(getComputedStyle(root).getPropertyValue('--gap-in'));
  if (!gapYIn && gapYIn !== 0) gapYIn = parseFloat(getComputedStyle(root).getPropertyValue('--gap-in'));
  var gapXPx = gapXIn * DPI;
  var gapYPx = gapYIn * DPI;
    var innerDiaPx = parseFloat(getComputedStyle(root).getPropertyValue('--inner-dia-in')) * DPI;
    var outerDiaPx = parseFloat(getComputedStyle(root).getPropertyValue('--outer-dia-in')) * DPI;
  var gridW = COLS * outerDiaPx + (COLS - 1) * gapXPx;
  var gridH = ROWS * outerDiaPx + (ROWS - 1) * gapYPx;
    var marginX = (pageWpx - gridW) / 2;
    var marginY = (pageHpx - gridH) / 2;
  return { DPI: DPI, pageWpx: pageWpx, pageHpx: pageHpx, gapXPx: gapXPx, gapYPx: gapYPx, innerDiaPx: innerDiaPx, outerDiaPx: outerDiaPx, marginX: marginX, marginY: marginY, gridW: gridW, gridH: gridH };
  }
  ...
  function coverContainRect(srcW, srcH, destW, destH, mode){
    ...
    if(mode === 'contain'){
      ...
      if(srcRatio > destRatio){
        var targetH = srcH; var targetW = targetH * destRatio; var sx = Math.floor((srcW - targetW)/2);
        return { sx: sx, sy: 0, sWidth: targetW, sHeight: targetH, dx: 0, dy: 0, dWidth: destW, dHeight: destH };
      } else {
        var targetW = srcW; var targetH = targetW / destRatio; var sy = Math.floor((srcH - targetH)/2);
  ...
  function applyTransformVars(page, index){
    ...
    var innerPx = parsePxVar(innerDiaStr, 192);
    var previewScale = parseFloat(previewScaleStr) || 0.9;
    var innerPxPreview = innerPx * previewScale;
    if (!ref.fitBox && ref.cell) {
      ref.fitBox = ref.cell.querySelector('.fit-box');
    }
    if (ref.fitBox) {
      ref.fitBox.style.setProperty('--scale', scale);
      ref.fitBox.style.setProperty('--tx', (nx * innerPxPreview) + 'px');
      ref.fitBox.style.setProperty('--ty', (ny * innerPxPreview) + 'px');
      ref.fitBox.style.cssText += `; --scale:${scale}; --tx:${nx * innerPxPreview}px; --ty:${ny * innerPxPreview}px;`;
    }
    if (ref.img) {
